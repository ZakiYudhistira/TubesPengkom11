    ...
    for row in range(1, sheet1.max_row+1):
        if userAcc == sheet1['A' + str(row)].value:
            ...
            if saldo < pay:
                ...
                saldo = saldo - pay
                Label(Page6, text="saldo anda saat ini adalah " + str(saldo), font=("Montserrat", 30, 'bold')).place(x=width/2-150, y=height/2-50)
                showFrame(Page6)
...
Button(Page6, text="QUIT", command=Quit).place(x=(width/2)-150, y=height-100)
