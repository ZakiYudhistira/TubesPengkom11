...
v = StringVar()
bawen = Button(Page3, image=bawenPhoto, textvariable=v, borderwidth=0, bg=bgcolor, padx=5, state=NORMAL); bawen.grid(row=2, column = 1); 
semarang = Button(Page3, image=semarangPhoto, textvariable=v, borderwidth=0, bg=bgcolor, padx=5, state=NORMAL); semarang.grid(row=2, column=2)
...
Label(Page3, text="Tol Jakarta", font=("Montserrat", 13, 'bold'), fg='#000000', bg=bgcolor).grid(row = 3, column = 1)

button = [bawen, semarang, solo, tboso, tbsumur, juanda, serpong, tjpriok, tmmini]
txtButton = ['bawen', 'semarang', 'solo', 'tambak oso']
...
def Login():
    ...
    try:
        auth.sign_in_with_email_and_password(loginEmail.get(), loginPass.get())
        # mail = mailSignin.get()
        # sheet2.cell(column=2, row=sheet2.max_row, value=waktuMasuk)
        Label(Page1, text="Successfully logged in!").pack()
        file.save(r'database.xlsx')
        success = True
    except:
...
def Signup():
    try:
        ...
        sheet1.cell(column=6, row=sheet1.max_row, value=saldo)
        file.save(r'database.xlsx')
        emailSignup.set(''); passSignup.set(''); saldoSignup.set(''); nameSignup.set('')
    except:
        failSignedUp = Label(Page1, text="Sign Up Failed"); failSignedUp.pack()
...
Label(Page1, text="Input ID", font= font).pack()
loginEmail = Entry(Page1, textvariable=mailSignin, width=100); loginEmail.pack(); user = mailSignin.get()
loginPass = Entry(Page1, width=100); loginPass.pack()
LoginButton = Button(Page1, text="Login", command=Login); LoginButton.pack()
...
bawen2 = Button(Page4, image=bawenPhoto, borderwidth=0, bg=bgcolor, padx=5, state=NORMAL); bawen.grid(row=2, column = 1); 
semarang2 = Button(Page4, image=semarangPhoto, borderwidth=0, bg=bgcolor, padx=5, state=NORMAL); semarang.grid(row=2, column=2)
solo2 = Button(Page4, image=soloPhoto, borderwidth=0, bg=bgcolor, padx=5, state=NORMAL); solo.grid(row=2, column=3)
tboso2 = Button(Page4, image=tbosoPhoto, borderwidth=0, bg=bgcolor, padx=5, state=NORMAL); tboso.grid(row=4, column=1)
tbsumur2 = Button(Page4, image=tbsumurPhoto, borderwidth=0, bg=bgcolor, padx=5, state=NORMAL); tbsumur.grid(row=4, column=2)
juanda2 = Button(Page4, image=juandaPhoto, borderwidth=0, bg=bgcolor, padx=5, state=NORMAL); juanda.grid(row=4, column=3)
serpong2 = Button(Page4, image=serpongPhoto, borderwidth=0, bg=bgcolor, padx=5, state=NORMAL); serpong.grid(row=6, column=1)
tjpriok2 = Button(Page4, image=tjpriokPhoto, borderwidth=0, bg=bgcolor, padx=5, state=NORMAL); tjpriok.grid(row=6, column=2)
tmmini2 = Button(Page4, image=tmminiPhoto, borderwidth=0, bg=bgcolor, padx=5, state=NORMAL); tmmini.grid(row=6, column=3)
...
def ClickedOut(indexTolKeluar):
    for i in range(len(button2)):
        button[i]['state'] = NORMAL
    button[indexTolKeluar]['state'] = DISABLED
    global exitNumber
...
def SubmitExit():
    for i in range(len(button2)):
        if i == exitNumber:
            ...
            sheet2.cell(column=9, row=sheet2.max_row, value=user)
